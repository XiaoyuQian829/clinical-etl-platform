...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Sidebar */}
      <aside className="flex flex-col w-[216px] shrink-0 h-full" style={{ background: "#51247A" }}>
        {/* Logo */}
              ...
              <button
                ...
                className={`flex items-center gap-2 w-full px-3 py-[9px] rounded-[5px] text-left text-[13px] transition-colors ${
                  active
                    ...
                    : "text-purple-200 hover:text-white hover:bg-[#5c2a8a]"
                }`}
      ...
      {/* Content */}
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto bg-[#f8f9fb]">
        {children}
      </main>
    </div>
  );
